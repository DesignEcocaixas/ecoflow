...
# MAGIA AQUI: Captura o lote enviado pelo Node.js (segundo argumento)
lote = sys.argv[2] if len(sys.argv) > 2 else None 
    ...
    try:
        ...
        if is_lisa or is_personalizada:
            cursor.execute(
                ...
                (cliente, vendedor, modelo, tamanho, quantidade, previsao_faturamento, lote)
        ...
        if is_personalizada:
            ...
            status = (
                ...
                if obs and str(obs).strip().upper() not in ["N/D", "NAN", "NONE"]
            ...
            cursor.execute(
                ...
                (cliente, vendedor, modelo, tamanho, material,
                 ...
                 status_pedido, previsao_faturamento, lote)
                ...
                (
                    ...
                    material,
                    cores,
                    obs if str(obs).strip().upper() not in ["NAN", "NONE"] else None,
                    int(qtd),
                    status,
                    previsao,
                    lote # <--- Lote inserido aqui
