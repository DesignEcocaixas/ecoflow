...
function termosComponent(user) {
    // Validação robusta: Aceita 1, "1" ou true
    if (user && (user.termos_aceitos === 1 || user.termos_aceitos === "1" || user.termos_aceitos === true)) {
        return '';
    }
    ...
    <style>
       @keyframes toastTermosSlideUp {
        ...
        @media (max-width: 575.98px) {
            #toastTermos { width: calc(100vw - 24px) !important; }
            #toastTermos .toast-body {
                flex-direction: column; align-items: stretch !important; text-align: center !important; padding: 1rem !important;
            }
            #toastTermos .toast-icon {
                margin-right: 0 !important; margin-bottom: 0.75rem; align-self: center; width: 42px !important; height: 42px !important;
            }
            #toastTermos .toast-content { padding-right: 0 !important; margin-bottom: 0.9rem; }
            #toastTermos .toast-actions { width: 100%; }
            #toastTermos .toast-actions .btn { width: 50%; }
        }
        .folha-a4 {
            background-color: #ffffff; width: 210mm; max-width: 100%; margin: 0 auto; padding: 40px 30px; border-radius: 4px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.4);
        }
        @media (max-width: 767.98px) { .folha-a4 { padding: 20px 15px; } }
        .termos-texto p, .termos-texto li { font-family: 'Times New Roman', Times, serif; font-size: 0.85rem; line-height: 1.6; color: #000000; text-align: justify; }
        .termos-texto h5, .termos-texto h6, .termos-texto strong { font-family: 'Times New Roman', Times, serif; color: #000000; }
        .termos-texto ul { padding-left: 1.5rem; }
        .termos-texto li { margin-bottom: 0.5rem; }
    </style>

    <div id="containerGeralTermos" class="toast-container position-fixed bottom-0 start-50 translate-middle-x p-3" style="z-index: 9999;">
                ...
                <div class="modal-body bg-custom-dark p-4">
                    ...
                    <div class="d-flex justify-content-center">
                        <div class="form-check d-inline-flex align-items-center bg-custom-darker p-3 rounded border-custom w-auto m-0 shadow-sm">
                            <input class="form-check-input flex-shrink-0 mt-0 me-3 ms-0 float-none" type="checkbox" id="checkAceiteTermos" onchange="alternarBotaoAceite(this)" style="cursor: pointer; border-color: rgba(255,255,255,0.3);">
                            <label class="form-check-label text-white fw-bold mb-0 text-start" for="checkAceiteTermos" style="cursor: pointer; font-family: 'Segoe UI', sans-serif; font-size: 0.85rem;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (toastEl) {
                const toast = new bootstrap.Toast(toastEl, { autohide: false, animation: false });
                setTimeout(() => {
                    if (toastMotion) {
        ...
        function fecharToastTermos() {
            const toastEl = document.getElementById('toastTermos');
            if (toastEl) {
                const toast = bootstrap.Toast.getInstance(toastEl) || new bootstrap.Toast(toastEl, { autohide: false, animation: false });
                toast.hide();
            }
        }
        ...
        function alternarBotaoAceite(checkbox) {
            document.getElementById('btnConcordarTermos').disabled = !checkbox.checked;
        }
        ...
        async function aceitarTermosSistema() {
            ...
            try {
                ...
                if (res.success) {
                    ...
                    // CORREÇÃO: Remove corretamente os elementos da tela instantaneamente
                    setTimeout(() => {
                        document.getElementById('containerGeralTermos')?.remove();
                        modalEl?.remove();
                    }, 300);
                } else {
